...
#include "TwoPhaseFlowVaporAdvectiveFlux.h"

registerMooseObject("HadesApp", TwoPhaseFlowVaporAdvectiveFlux);

InputParameters
TwoPhaseFlowVaporAdvectiveFlux::validParams()
{
...
TwoPhaseFlowVaporAdvectiveFlux::TwoPhaseFlowVaporAdvectiveFlux(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
TwoPhaseFlowVaporAdvectiveFlux::computeQpResidual()
{
...
Real
TwoPhaseFlowVaporAdvectiveFlux::computeQpJacobian()
{
  Real R = 8.314; // Universal gas constant [J/(kgK)]
  Real Mw = 0.01801528; // Molar mass of water [kg/mol]
  Real Ma = 0.0289647; // Molar mass of dry air [kg/mol]
...
// Jacobian term
  Real dm = _grad_test[_i][_qp] * dvapor_density_dPcap * _IPerm[_qp] * _RPerm[_qp] / _Gviscosity[_qp] * (_grad_Pgas[_qp] - gas_density * _gravity);
      dm -= _grad_test[_i][_qp] * vapor_density * _IPerm[_qp] * _RPerm[_qp] / _Gviscosity[_qp] * dgas_density_dPcap * _gravity;

  return dm; 
}

Real
TwoPhaseFlowVaporAdvectiveFlux::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real R = 8.314; // Universal gas constant [J/(kgK)]
  Real Mw = 0.01801528; // Molar mass of water [kg/mol]
  Real Ma = 0.0289647; // Molar mass of dry air [kg/mol]
  ...
  Real dm_T = _grad_test[_i][_qp] * dvapor_density_dT * _IPerm[_qp] * _RPerm[_qp] / _Gviscosity[_qp] * (_grad_Pgas[_qp] - gas_density * _gravity);
      dm_T -= _grad_test[_i][_qp] * vapor_density * _IPerm[_qp] * _RPerm[_qp] / _Gviscosity[_qp] * (dgas_density_dT * _gravity);

  Real dm_Pgas = _grad_test[_i][_qp] * vapor_density * _IPerm[_qp] * _RPerm[_qp] / _Gviscosity[_qp] * (_grad_phi[_j][_qp] - dgas_density_dPgas * _gravity);
