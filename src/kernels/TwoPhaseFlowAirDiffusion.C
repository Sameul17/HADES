...
#include "TwoPhaseFlowAirDiffusion.h"

registerMooseObject("HadesApp", TwoPhaseFlowAirDiffusion);

InputParameters
TwoPhaseFlowAirDiffusion::validParams()
{
  InputParameters params = Kernel::validParams();

  params.addCoupledVar("Temp", 300, "The temperature of each quadrature point");
  params.addCoupledVar("Pcap", -1E6, "The gas pressure of each quadrature point");
...
TwoPhaseFlowAirDiffusion::TwoPhaseFlowAirDiffusion(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
TwoPhaseFlowAirDiffusion::computeQpResidual()
{
...
//  Real drho_dT = 4976 / _T[_qp] * rho_vS; // The derivative term of drho_vs / dtemp
  ...
  Real f_Tv = 1; //The thermal diffusion enhancement factor for the thermal diffusion term (f_Tv = 1 + T / 4976)
...
Real
TwoPhaseFlowAirDiffusion::computeQpJacobian()
{
  Real R = 8.314; // Gas cosntant [J/(molK)]
  Real Mw = 0.01801528; // Molar mass of water [kg/mol]
  Real Rv = 461.5; // The specific gas constant [J/(kgK)]
...
Real
TwoPhaseFlowAirDiffusion::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real R = 8.314; // Gas cosntant [J/(molK)]
  Real Mw = 0.01801528; // Molar mass of water [kg/mol]
  Real Rv = 461.5; // The specific gas constant [J/(kgK)]
