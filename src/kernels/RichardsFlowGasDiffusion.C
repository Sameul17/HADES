...
#include "RichardsFlowGasDiffusion.h"

registerMooseObject("HadesApp", RichardsFlowGasDiffusion);

InputParameters
RichardsFlowGasDiffusion::validParams()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("tortuosity", "The Bentonite porosity of each quadrature point");
...
RichardsFlowGasDiffusion::RichardsFlowGasDiffusion(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
RichardsFlowGasDiffusion::computeQpResidual()
{
...
//  Real D_v = 5E-11 * 365 * 24 * 3600;
  Real D_v = 2.16E-5 * _tortuosity[_qp] * _porosity[_qp] * (1 - Sat) * pow((_T[_qp] / 273),1.8) * 365 * 24 * 3600 / 10; // The effective vapor diffusion coefficient [m2/yr]
//  Real D_v = 2.15E-5 * pow(_T[_qp] + 273.15, 1.8) * 365 * 24 *3600;
...
Real
RichardsFlowGasDiffusion::computeQpJacobian()
{
  Real R = 8.314; // Gas cosntant [J/(molK)]
  Real Mw = 0.01801528; // Molar mass of water [kg/mol]
  Real Rv = 461.5; // The specific gas constant [J/(kgK)]
  ...
  Real Sat = pow(1 + pow((-_alpha[_qp] * _u[_qp]),(1 / (1 - _lambda[_qp]))),(-_lambda[_qp]));
  Real D_v = 2.16E-5 * _tortuosity[_qp] * _porosity[_qp] * (1 - Sat) * pow((_T[_qp] / 273),1.8) * 365 * 24 * 3600 /10; // The effective vapor diffusion coefficient [m2/yr]
//  Real D_v = 5.9E-12 * pow(_T[_qp] + 273.15, 2.3) / 0.101 * 365 * 24 *3600;
...
Real
RichardsFlowGasDiffusion::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real R = 8.314; // Gas cosntant [J/(molK)]
  Real Mw = 0.01801528; // Molar mass of water [kg/mol]
  Real Rv = 461.5; // The specific gas constant [J/(kgK)]
