...
#include "SatCalculation.h"

registerMooseObject("HadesApp", SatCalculation);

InputParameters
SatCalculation::validParams()
{
...
SatCalculation::SatCalculation(const InputParameters & parameters)
 ...
 */
Real
SatCalculation::computeValue()
{
  return  pow((1 + pow((-_alpha[_qp] * _Pwater[_qp]), (1 / (1 - _lambda[_qp])))),(-_lambda[_qp]));
}
