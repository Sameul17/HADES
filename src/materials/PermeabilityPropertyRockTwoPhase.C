...
#include "PermeabilityPropertyRockTwoPhase.h"

registerMooseObject("HadesApp", PermeabilityPropertyRockTwoPhase);

InputParameters
PermeabilityPropertyRockTwoPhase::validParams()
{
...
PermeabilityPropertyRockTwoPhase::PermeabilityPropertyRockTwoPhase(const InputParameters & parameters)
  : Material(parameters),
...
void
PermeabilityPropertyRockTwoPhase::initQpStatefulProperties()
{
  Real Sat = pow((1 + pow((-_alpha[_qp] * _P[_qp]),(1 / (1 - _lambda[_qp])))),(-_lambda[_qp]));

  _Rperm_l[_qp] = pow(Sat,0.5) * pow(1 - pow(1 - pow(Sat,1/0.6),0.6),2);
  _Rperm_g[_qp] = 1 - _Rperm_l[_qp];
}

void
PermeabilityPropertyRockTwoPhase::computeQpProperties()
{
  Real Sat = pow((1 + pow((-_alpha[_qp] * _P[_qp]),(1 / (1 - _lambda[_qp])))),(-_lambda[_qp]));
