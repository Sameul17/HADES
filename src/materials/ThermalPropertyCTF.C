...
#include "ThermalPropertyCTF.h"

registerMooseObject("HadesApp", ThermalPropertyCTF);

InputParameters
ThermalPropertyCTF::validParams()
{
...
ThermalPropertyCTF::ThermalPropertyCTF(const InputParameters & parameters)
  : Material(parameters),
...
void
ThermalPropertyCTF::initQpStatefulProperties()
{
  ...
  _Thermal_conductivity[_qp] = (_dry_thermal_conductivity, 1 - Sat) * (_wet_thermal_conductivity, Sat);
}

void
ThermalPropertyCTF::computeQpProperties()
{
  Real Sat = pow((1 + pow((-_alpha[_qp] * _P[_qp]),(1 / (1 - _lambda[_qp])))),(-_lambda[_qp]));
  Real StoY = 3600 * 24 * 365;
