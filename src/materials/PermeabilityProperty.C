...
registerMooseObject("HadesApp", PermeabilityProperty);

InputParameters
PermeabilityProperty::validParams()
{
  InputParameters params = Material::validParams();
  params.addRequiredCoupledVar("Pwater","Put the water pressure variable");
...
PermeabilityProperty::PermeabilityProperty(const InputParameters & parameters)
  : Material(parameters),
...
void
PermeabilityProperty::initQpStatefulProperties()
{
...
void
PermeabilityProperty::computeQpProperties()
{
  Real Sat = pow((1 + pow((-_alpha[_qp] * _P[_qp]),(1 / (1 - _lambda[_qp])))),(-_lambda[_qp]));
