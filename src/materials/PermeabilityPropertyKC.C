...
#include "PermeabilityPropertyKC.h"

registerMooseObject("HadesApp", PermeabilityPropertyKC);

InputParameters
PermeabilityPropertyKC::validParams()
{
...
PermeabilityPropertyKC::PermeabilityPropertyKC(const InputParameters & parameters)
  : Material(parameters),
...
void
PermeabilityPropertyKC::initQpStatefulProperties()
{
...
void
PermeabilityPropertyKC::computeQpProperties()
{
  Real Sat = pow((1 + pow((-_alpha[_qp] * _P[_qp]),(1 / (1 - _lambda[_qp])))),(-_lambda[_qp]));
