...
#include "ThermalPropertyTwoPhase.h"

registerMooseObject("HadesApp", ThermalPropertyTwoPhase);

InputParameters
ThermalPropertyTwoPhase::validParams()
{
...
ThermalPropertyTwoPhase::ThermalPropertyTwoPhase(const InputParameters & parameters)
  : Material(parameters),
...
void
ThermalPropertyTwoPhase::initQpStatefulProperties()
{
...
void
ThermalPropertyTwoPhase::computeQpProperties()
{
  Real Sat = pow((1 + pow((-_alpha[_qp] * _P[_qp]),(1 / (1 - _lambda[_qp])))),(-_lambda[_qp]));
  Real StoY = 3600 * 24 * 365;
