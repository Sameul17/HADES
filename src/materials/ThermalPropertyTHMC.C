...
#include "ThermalPropertyTHMC.h"

registerMooseObject("HadesApp", ThermalPropertyTHMC);

InputParameters
ThermalPropertyTHMC::validParams()
{
...
ThermalPropertyTHMC::ThermalPropertyTHMC(const InputParameters & parameters)
  : Material(parameters),
...
void
ThermalPropertyTHMC::initQpStatefulProperties()
{
  ...
  _Thermal_conductivity[_qp] = (1.28 - 0.71 / (1 + exp((Sat - 0.65) / 0.1))) * StoY;
}

void
ThermalPropertyTHMC::computeQpProperties()
{
  Real Sat = pow((1 + pow((-_alpha[_qp] * _P[_qp]),(1 / (1 - _lambda[_qp])))),(-_lambda[_qp]));
  Real StoY = 3600 * 24 * 365;
